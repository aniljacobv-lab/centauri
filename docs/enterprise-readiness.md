...
| Enterprise SSO (OIDC / JWT bearer auth) | ✓ | `serve -oidc-issuer … [-oidc-audience … -oidc-jwks … -oidc-write-scope …]` accepts bearer tokens issued by any standard OIDC provider (Okta, Azure AD, Auth0, Keycloak, Google, …) **alongside** the static tokens. Centauri verifies the RS256/384/512 signature against the provider's JWKS (fetched over HTTPS, key-cached, refreshed on rotation), and checks `exp`/`nbf`/`iss`/`aud`. A valid token grants read; **write requires the configured `-oidc-write-scope`** (matched against the token's `scope`/`roles`/`groups`), so any employee can read but only the right group can write. Zero third-party deps — `crypto/rsa`, `crypto/sha256`, `net/http`, `math/big` only (`internal/oidc`); Centauri validates the IdP's tokens, it does **not** run an identity provider. Still token-only on `serve -lazy-index`. |
...
| Capability | Status | Notes |
|---|---|---|
| SQL (read-only SELECT subset) | partial | A lean SQL `SELECT` (WHERE / GROUP BY / HAVING / ORDER BY / LIMIT, plus `AS OF` and SQL:2011 `FOR SYSTEM_TIME AS OF`) transpiles to CeQL at `POST/GET /v1/sql` and over a **PostgreSQL wire-protocol listener** (`serve -pg-addr :5432`) — so `psql`, JDBC/ODBC drivers, DBeaver, Tableau, and Power BI can connect to Centauri directly (`sslmode=disable`, any user, password = the `-token`/`-read-token`) and run read-only `SELECT`s. Zero third-party deps — the listener is `net` + `encoding/binary` over the existing SQL→CeQL transpiler (`internal/pgwire`). Both the **simple** and the **extended** (Parse/Bind/Describe/Execute) query sub-protocols are implemented, so prepared statements with parameters work — JDBC and psycopg connect in their default mode (no `preferQueryMode=simple` needed). Parameters arrive as text or binary wire values, are rendered to SQL literals (numeric/boolean unquoted, others quoted), and are substituted before transpiling. Limits (honest): every column is returned as `TEXT` (OID 25); `Describe` runs the statement with placeholder values to learn its column shape; there is no `?db=` routing (the default database is served); and **writes still use CeQL** (the wire path rejects non-`SELECT`). |
...
| Row-level security + field masking | partial | Scoped tokens confine a principal to subject prefixes (RLS), and a token's policy can now carry a `mask` list that **redacts those value fields** ("***") in its query results (VPD-style column masking; the store's data is never mutated — masked copies are returned). Set via `POST /v1/acl` with `mask`. **OIDC/JWT SSO is now supported** (see the auth row above — IdP-issued tokens carry their own expiry and rotate via JWKS). Still missing: full role hierarchies, LDAP/SAML, and mapping individual SSO claims to per-subject ACL scopes (today an SSO token is read, or write with the write scope). |
