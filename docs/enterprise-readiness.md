...
| Object-store cold tier (S3/GCS) | partial | `internal/objstore` is a pluggable `SegmentStore` — local dir or **S3-compatible** (AWS S3 / MinIO / R2 / B2) via stdlib SigV4, no SDK. `centauri archive-push` uploads a sealed archive to your own bucket (and `-pull` restores it). Not yet: serving reads *directly from* object storage (the lazy reader still reads local files) — for now pull the archive local, then `serve -lazy-index`. SigV4 interop should be smoke-tested against your store (MinIO/real S3). |
