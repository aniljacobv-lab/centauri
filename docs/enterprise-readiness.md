...
| Write throughput under concurrency | partial | `-group-commit` (opt-in) lets concurrent `Append` calls coalesce into **one fsync** per batch (throughput scales with batch size, not one fsync per caller) while keeping the single ordered chain. True horizontal write scaling = sharding by subject namespace across independent logs (each its own chain) — planned, not yet built. |
