...
func Entries() []Entry {
	...
	return []Entry{
		...
		{"read", "profile", "PROFILE OF <pattern>", "PROFILE OF item:*",
			"What does my data look like? Field types, coverage, ranges, top values, facet/namespace distribution — time-travel aware",
			[]string{"profile", "data shape", "what does my data look like", "describe the data", "summary of", "explore"}},
		...
		{"agg", "distinct", "FACTS COUNT(DISTINCT <field>) OF <pattern>", "FACTS COUNT(DISTINCT facet) OF item:*/store:4001",
			"How many distinct values — exact, or APPROX_COUNT_DISTINCT(field) for HyperLogLog at scale",
			[]string{"distinct", "count distinct", "unique values", "cardinality", "how many different"}},
		...
		// ---- TOPOLOGY (the differentiator) ----
		{"topology", "shape", "SHAPE OF <pattern> ON <field>[, <field>]", "SHAPE OF item:*/store:4001 ON price_cents",
			"Persistent homology of a value cloud: connected components (Betti-0) and loops (Betti-1) no row store can see",
			[]string{"shape", "topology", "betti", "clusters", "loops", "structure", "homology", "persistent"}},
		{"topology", "shape-2d", "SHAPE OF <pattern> ON <f1>, <f2> SCALE <n>", "SHAPE OF item:* ON price_cents, trust SCALE 100",
			"Multi-field shape — axes are auto z-score normalized so units don't dominate (add RAW to disable)",
			[]string{"shape in 2d", "two fields shape", "scatter topology", "high dimensional shape", "normalize"}},
		{"topology", "shape-periodicity", "SHAPE OF <subject> ON <field> WINDOW <n>", "SHAPE OF item:100001/store:4001 ON price_cents WINDOW 12",
			"Detect periodicity/seasonality: a time-delay embedding turns a periodic signal into a loop (Betti-1>0). The SW1PerS method",
			[]string{"periodicity", "seasonality", "cycle in signal", "is it periodic", "sliding window", "sw1pers", "recurring pattern"}},
		{"topology", "shape-embedding", "SHAPE OF <pattern> ON EMBEDDING METRIC cosine", "SHAPE OF item:* ON EMBEDDING METRIC cosine",
			"Topology of the semantic embedding cloud (cosine metric): clusters and loops in vector space",
			[]string{"embedding shape", "vector space topology", "semantic clusters", "embedding structure"}},
		{"topology", "shape-voids", "SHAPE OF <pattern> ON <f1>, <f2>, <f3> MAXDIM 2", "SHAPE OF item:* ON price_cents, trust, av_cost MAXDIM 2",
			"Up to Betti-2 — detect voids / coverage holes in higher-dimensional clouds",
			[]string{"voids", "coverage holes", "betti 2", "cavities", "3d shape"}},
		{"topology", "consistency", "CONSISTENCY OF <subject> ON <field>", "CONSISTENCY OF " + sub + " ON price_cents",
			"Sheaf consistency of a subject's facets: agreement clusters, an inconsistency score, and the outlier facet",
			[]string{"consistency", "do facets agree", "sheaf", "reconcile", "agreement", "outlier facet"}},
		{"topology", "consistency-eps", "CONSISTENCY OF <subject> ON <field> EPS <n>", "CONSISTENCY OF " + sub + " ON price_cents EPS 5",
			"Consistency with a tolerance: facets within EPS count as agreeing",
			[]string{"consistency tolerance", "close enough", "within eps"}},
		{"topology", "cycles", "CYCLES IN CAUSES [OF <subject>]", "CYCLES IN CAUSES OF " + sub,
			"Detect directed cycles in the causal graph — lineage must be acyclic, so any cycle is an integrity alarm",
			[]string{"cycles", "causal loop", "lineage integrity", "circular cause", "acyclic check"}},
		{"topology", "drift", "DRIFT OF <pattern> ON <field> BUCKETS <n>", "DRIFT OF item:*/store:4001 ON price_cents BUCKETS 6",
			"Topological drift: how the distribution of a field changes across time buckets (regime-change detector)",
			[]string{"drift", "regime change", "distribution change", "shift over time", "concept drift"}},

		// ---- SEARCH ----
		{"search", "fulltext", "SEARCH '<text>' OF <pattern>", "SEARCH 'late markdown' OF item:*",
			"Ranked full-text search (BM25) across subjects and text values — relevance-ordered, not just substring match",
			[]string{"search", "find", "full text", "keyword", "lookup", "ranked search", "bm25", "look for"}},
		{"search", "hybrid", "SEARCH '<text>' OF <pattern> SIMILAR TO <event> ALPHA <0..1>", "SEARCH 'markdown' OF item:* SIMILAR TO 0193fa2e-77c1 ALPHA 0.5",
			"Hybrid keyword + vector ranking: blends BM25 with embedding similarity (ALPHA 1=keyword .. 0=semantic)",
			[]string{"hybrid search", "keyword and vector", "semantic plus keyword", "rerank", "rag retrieval"}},
		{"search", "fulltext-time", "SEARCH '<text>' OF <pattern> AS KNOWN AT <t>", "SEARCH 'penny' OF item:* AS KNOWN AT '2026-03-01'",
			"Search the corpus as it was known at a past moment — full-text is bi-temporal too",
			[]string{"search the past", "what matched then", "historical search"}},
